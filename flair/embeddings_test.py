...
class BPEmbSerializable(BPEmb):
    ...
    def __setstate__(self, state):
        ...
        self.cache_dir       = Path(flair.file_utils.CACHE_ROOT) / 'embeddings'
...
class BytePairEmbeddings(TokenEmbeddings):
    ...
    def __init__(self, language     , dim      = 50, syllables      = 100000, cache_dir = Path(flair.file_utils.CACHE_ROOT) / 'embeddings'):
        """
        Initializes BP embeddings. Constructor downloads required files if not there.
        """

        self.name      = f'bpe-{language}-{syllables}-{dim}'
        self.static_embeddings = True
...
class PooledFlairEmbeddings(TokenEmbeddings):
    ...
    def __init__(self,
        ...
        if type(contextual_embeddings) is str:
            self.context_embeddings                  = FlairEmbeddings(contextual_embeddings, **kwargs)
        else:
...
class DocumentPoolEmbeddings(DocumentEmbeddings):
    ...
    def __init__(self, embeddings                       , mode      = 'mean'):
        """The constructor takes a list of embeddings to be combined.
        ...
        self.name      = f'document_{self.mode}'
...
class DocumentLMEmbeddings(DocumentEmbeddings):
    def __init__(self, flair_embeddings                       , detach       = True):
        ...
        self._embedding_length      = sum(embedding.embedding_length for embedding in flair_embeddings)
