...
class TextClassifier(flair.nn.Model):
    ...
    def __init__(self, document_embeddings: flair.embeddings.DocumentEmbeddings, label_dictionary: Dictionary, multi_label: bool):
    ...
    def load(model: str):
        ...
        if (model.lower() == 'de-offensive-language'):
            base_path = '/'.join([aws_resource_path,
                                  'TEXT-CLASSIFICATION_germ-eval-2018_task-1', 'germ-eval-2018-task-1.pt'])
            model_file = cached_path(base_path, cache_dir=cache_dir)
        elif (model.lower() == 'en-sentiment'):
            base_path = '/'.join([aws_resource_path,
                                  'TEXT-CLASSIFICATION_imdb', 'imdb.pt'])
            model_file = cached_path(base_path, cache_dir=cache_dir)
