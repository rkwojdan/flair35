...
class Metric(object):
    ...
    def accuracy(self, class_name=None):
        if ((((self.get_tp(class_name) + self.get_tn(class_name)) + self.get_fp(class_name)) + self.get_fn(class_name)) > 0):
            return round(((self.get_tp(class_name) + self.get_tn(class_name)) / (((self.get_tp(class_name) + self.get_tn(class_name)) + self.get_fp(class_name)) + self.get_fn(class_name))), 4)
        return 0.0
    ...
    def macro_avg_f_score(self):
        class_precisions = [self.precision(class_name)
                            for class_name in self.get_classes()]
        class_recalls = [self.recall(class_name)
                         for class_name in self.get_classes()]
    ...
    def get_classes(self):
        all_classes = set(itertools.chain(*list(list([list(keys) for keys in [
                          self._tps.keys(), self._fps.keys(), self._tns.keys(), self._fns.keys()]]))))
    ...
    def to_tsv(self):
        return u'{}\t{}\t{}\t{}'.format(self.precision(), self.recall(), self.accuracy(), self.micro_avg_f_score())

    @staticmethod
    def tsv_header(prefix=None):
        if prefix:
            return u'{0}_PRECISION\t{0}_RECALL\t{0}_ACCURACY\t{0}_F-SCORE'.format(prefix)
        return u'PRECISION\tRECALL\tACCURACY\tF-SCORE'
    ...
    def __str__(self):
        ...
        all_lines = [u'{0:<10}\ttp: {1} - fp: {2} - fn: {3} - tn: {4} - precision: {5:.4f} - recall: {6:.4f} - accuracy: {7:.4f} - f1-score: {8:.4f}'.format((self.name if (class_name is None) else class_name), self.get_tp(
...
class EvaluationMetric(Enum):
    MICRO_ACCURACY = u'micro-average accuracy'
    MICRO_F1_SCORE = u'micro-average f1-score'
    MACRO_ACCURACY = u'macro-average accuracy'
    MACRO_F1_SCORE = u'macro-average f1-score'


class WeightExtractor(object):

    def __init__(self, directory, number_of_weights=10):
        self.weights_file = init_output_file(directory, u'weights.txt')
        ...
        for key in state_dict.keys():
            ...
            for i in range(weights_to_watch):
                ...
                for index in self.weights_dict[key][i]:
                    vec = vec[index]
                value = vec.item()
                with open(self.weights_file, u'a') as f:
                    f.write(u'{}\t{}\t{}\t{}\n'.format(
...
def clear_embeddings(sentences, also_clear_word_embeddings=False):
    u'\n    Clears the embeddings from all given sentences.\n    :param sentences: list of sentences\n    '
    for sentence in sentences:
        sentence.clear_embeddings(
            also_clear_word_embeddings=also_clear_word_embeddings)


def init_output_file(base_path, file_name):
    u'\n    Creates a local file.\n    :param base_path: the path to the directory\n    :param file_name: the file name\n    :return: the created file\n    '
    base_path.mkdir(parents=True, exist_ok=True)
    file = (base_path / file_name)
    open(file, u'w', encoding=u'utf-8').close()
    return file


def convert_labels_to_one_hot(label_list, label_dict):
...
def add_file_handler(log, output_file):
    ...
    formatter = logging.Formatter(u'%(asctime)-15s %(message)s')
